...
# Enforce: if a PR adds migration files under drizzle/migrations/, it must add <= 1.
...
# Be slightly strict: avoid counting non-migration sql files.
# Drizzle migrations typically look like: <timestamp>_<slug>.sql
MIGRATIONS=$(echo "$ADDED_FILES" | grep -E '^drizzle/migrations/[0-9]+_.*\.sql$' || true)
...
echo "- If you have multiple Drizzle migrations, create ONE consolidated migration instead of several."
