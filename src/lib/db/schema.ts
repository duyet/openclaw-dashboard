...
export const organizationMembers = sqliteTable('organization_members', {
  ...
  allBoardsRead: integer('all_boards_read', { mode: 'boolean' }).notNull().default(false),
  allBoardsWrite: integer('all_boards_write', { mode: 'boolean' }).notNull().default(false),
  createdAt: createdAt(),
  updatedAt: updatedAt(),
}, (table) => [
...
export const organizationInvites = sqliteTable('organization_invites', {
  ...
  index('idx_organization_invites_created_by_user_id').on(table.createdByUserId),
  index('idx_organization_invites_accepted_by_user_id').on(table.acceptedByUserId),
]);
...
export const gateways = sqliteTable('gateways', {
  ...
  index('idx_gateways_organization_id').on(table.organizationId),
]);
...
export const boards = sqliteTable('boards', {
  ...
  successMetrics: text('success_metrics').$type<Record<string, unknown> | null>(),
  targetDate: text('target_date'),
  goalConfirmed: integer('goal_confirmed', { mode: 'boolean' }).notNull().default(false),
  goalSource: text('goal_source'),
  requireApprovalForDone: integer('require_approval_for_done', { mode: 'boolean' }).notNull().default(true),
  requireReviewBeforeDone: integer('require_review_before_done', { mode: 'boolean' }).notNull().default(false),
  blockStatusChangesWithPendingApproval: integer('block_status_changes_with_pending_approval', { mode: 'boolean' }).notNull().default(false),
  onlyLeadCanChangeStatus: integer('only_lead_can_change_status', { mode: 'boolean' }).notNull().default(false),
...
// ---------------------------------------------------------------------------
export const organizationBoardAccess = sqliteTable('organization_board_access', {
  id: uuidPk(),
  organizationMemberId: text('organization_member_id').notNull().references(() => organizationMembers.id),
  ...
  uniqueIndex('uq_org_board_access_member_board').on(table.organizationMemberId, table.boardId),
  index('idx_org_board_access_organization_member_id').on(table.organizationMemberId),
...
// ---------------------------------------------------------------------------
export const organizationInviteBoardAccess = sqliteTable('organization_invite_board_access', {
  id: uuidPk(),
  organizationInviteId: text('organization_invite_id').notNull().references(() => organizationInvites.id),
  boardId: text('board_id').notNull().references(() => boards.id),
  canRead: integer('can_read', { mode: 'boolean' }).notNull().default(true),
  canWrite: integer('can_write', { mode: 'boolean' }).notNull().default(false),
  createdAt: createdAt(),
  ...
  uniqueIndex('uq_org_invite_board_access_invite_board').on(table.organizationInviteId, table.boardId),
  index('idx_org_invite_board_access_organization_invite_id').on(table.organizationInviteId),
...
export const agents = sqliteTable('agents', {
  ...
  name: text('name').notNull(),
  status: text('status', { enum: ['provisioning', 'online', 'offline', 'updating', 'deleting', 'deleted'] }).notNull().default('provisioning'),
  openclawSessionId: text('openclaw_session_id'),
  agentTokenHash: text('agent_token_hash'),
  heartbeatConfig: text('heartbeat_config').$type<Record<string, unknown> | null>(),
  identityProfile: text('identity_profile').$type<Record<string, unknown> | null>(),
  ...
  isBoardLead: integer('is_board_lead', { mode: 'boolean' }).notNull().default(false),
  createdAt: createdAt(),
  updatedAt: updatedAt(),
}, (table) => [
  ...
  index('idx_agents_provision_confirm_token_hash').on(table.provisionConfirmTokenHash),
  index('idx_agents_provision_action').on(table.provisionAction),
  index('idx_agents_delete_confirm_token_hash').on(table.deleteConfirmTokenHash),
...
export const tasks = sqliteTable('tasks', {
  ...
  description: text('description'),
  status: text('status', { enum: ['inbox', 'in_progress', 'review', 'done', 'blocked', 'cancelled'] }).notNull().default('inbox'),
...
export const taskDependencies = sqliteTable('task_dependencies', {
  ...
  uniqueIndex('uq_task_dependencies_task_id_depends_on_task_id').on(table.taskId, table.dependsOnTaskId),
...
// ---------------------------------------------------------------------------
export const taskFingerprints = sqliteTable('task_fingerprints', {
  id: uuidPk(),
  boardId: text('board_id').notNull().references(() => boards.id),
...
// ---------------------------------------------------------------------------
export const approvalTaskLinks = sqliteTable('approval_task_links', {
  id: uuidPk(),
  approvalId: text('approval_id').notNull().references(() => approvals.id),
  taskId: text('task_id').notNull().references(() => tasks.id),
  createdAt: createdAt(),
}, (table) => [
  uniqueIndex('uq_approval_task_links_approval_id_task_id').on(table.approvalId, table.taskId),
...
// 16. board_memory
// ---------------------------------------------------------------------------
export const boardMemory = sqliteTable('board_memory', {
  id: uuidPk(),
  boardId: text('board_id').notNull().references(() => boards.id),
...
// ---------------------------------------------------------------------------
export const boardGroupMemory = sqliteTable('board_group_memory', {
  id: uuidPk(),
  boardGroupId: text('board_group_id').notNull().references(() => boardGroups.id),
  content: text('content').notNull(),
  tags: text('tags').$type<string[] | null>(),
  isChat: integer('is_chat', { mode: 'boolean' }).notNull().default(false),
  source: text('source'),
...
// ---------------------------------------------------------------------------
export const boardOnboardingSessions = sqliteTable('board_onboarding_sessions', {
  id: uuidPk(),
  boardId: text('board_id').notNull().references(() => boards.id),
...
// 19. board_webhooks
// ---------------------------------------------------------------------------
export const boardWebhooks = sqliteTable('board_webhooks', {
  id: uuidPk(),
  boardId: text('board_id').notNull().references(() => boards.id),
  agentId: text('agent_id').references(() => agents.id),
...
// ---------------------------------------------------------------------------
export const boardWebhookPayloads = sqliteTable('board_webhook_payloads', {
  id: uuidPk(),
  boardId: text('board_id').notNull().references(() => boards.id),
  webhookId: text('webhook_id').notNull().references(() => boardWebhooks.id),
...
export const tags = sqliteTable('tags', {
  ...
  uniqueIndex('uq_tags_organization_id_slug').on(table.organizationId, table.slug),
...
export const tagAssignments = sqliteTable('tag_assignments', {
  ...
  uniqueIndex('uq_tag_assignments_task_id_tag_id').on(table.taskId, table.tagId),
...
// ---------------------------------------------------------------------------
export const taskCustomFieldDefinitions = sqliteTable('task_custom_field_definitions', {
  id: uuidPk(),
  organizationId: text('organization_id').notNull().references(() => organizations.id),
  fieldKey: text('field_key').notNull(),
  label: text('label').notNull(),
  fieldType: text('field_type', {
    enum: ['text', 'text_long', 'integer', 'decimal', 'boolean', 'date', 'date_time', 'url', 'json'],
  ...
  uniqueIndex('uq_task_custom_field_definitions_org_id_field_key').on(table.organizationId, table.fieldKey),
  index('idx_task_custom_field_definitions_organization_id').on(table.organizationId),
...
// ---------------------------------------------------------------------------
export const taskCustomFieldValues = sqliteTable('task_custom_field_values', {
  id: uuidPk(),
  taskId: text('task_id').notNull().references(() => tasks.id),
  taskCustomFieldDefinitionId: text('task_custom_field_definition_id').notNull().references(() => taskCustomFieldDefinitions.id),
  value: text('value').$type<unknown>(),
  createdAt: createdAt(),
  updatedAt: updatedAt(),
}, (table) => [
  uniqueIndex('uq_task_custom_field_values_task_id_task_custom_field_definition_id').on(table.taskId, table.taskCustomFieldDefinitionId),
  index('idx_task_custom_field_values_task_id').on(table.taskId),
  index('idx_task_custom_field_values_task_custom_field_definition_id').on(table.taskCustomFieldDefinitionId),
]);

// ---------------------------------------------------------------------------
// 26. board_task_custom_fields
// ---------------------------------------------------------------------------
export const boardTaskCustomFields = sqliteTable('board_task_custom_fields', {
  id: uuidPk(),
  boardId: text('board_id').notNull().references(() => boards.id),
  taskCustomFieldDefinitionId: text('task_custom_field_definition_id').notNull().references(() => taskCustomFieldDefinitions.id),
  createdAt: createdAt(),
}, (table) => [
  uniqueIndex('uq_board_task_custom_fields_board_id_task_custom_field_definition_id').on(table.boardId, table.taskCustomFieldDefinitionId),
  index('idx_board_task_custom_fields_board_id').on(table.boardId),
  index('idx_board_task_custom_fields_task_custom_field_definition_id').on(table.taskCustomFieldDefinitionId),
]);
...
export const marketplaceSkills = sqliteTable('marketplace_skills', {
  ...
  metadata: text('metadata').notNull().$type<Record<string, unknown>>().$defaultFn(() => JSON.stringify({}) as unknown as Record<string, unknown>),
  createdAt: createdAt(),
  updatedAt: updatedAt(),
}, (table) => [
  uniqueIndex('uq_marketplace_skills_org_source_url').on(table.organizationId, table.sourceUrl),
...
export const skillPacks = sqliteTable('skill_packs', {
  ...
  uniqueIndex('uq_skill_packs_org_source_url').on(table.organizationId, table.sourceUrl),
...
// ---------------------------------------------------------------------------
export const gatewayInstalledSkills = sqliteTable('gateway_installed_skills', {
  id: uuidPk(),
  gatewayId: text('gateway_id').notNull().references(() => gateways.id),
  skillId: text('skill_id').notNull().references(() => marketplaceSkills.id),
  createdAt: createdAt(),
  updatedAt: updatedAt(),
}, (table) => [
  uniqueIndex('uq_gateway_installed_skills_gateway_id_skill_id').on(table.gatewayId, table.skillId),
...
export const organizationInvitesRelations = relations(organizationInvites, ({ one, many }) => ({
  organization: one(organizations, {
...
export const organizationBoardAccessRelations = relations(organizationBoardAccess, ({ one }) => ({
...
export const organizationInviteBoardAccessRelations = relations(organizationInviteBoardAccess, ({ one }) => ({
...
export const taskFingerprintsRelations = relations(taskFingerprints, ({ one }) => ({
  board: one(boards, {
...
export const approvalTaskLinksRelations = relations(approvalTaskLinks, ({ one }) => ({
...
export const boardGroupMemoryRelations = relations(boardGroupMemory, ({ one }) => ({
  boardGroup: one(boardGroups, {
...
export const boardOnboardingSessionsRelations = relations(boardOnboardingSessions, ({ one }) => ({
  board: one(boards, {
...
export const boardWebhooksRelations = relations(boardWebhooks, ({ one, many }) => ({
  board: one(boards, {
...
export const boardWebhookPayloadsRelations = relations(boardWebhookPayloads, ({ one }) => ({
  board: one(boards, {
...
export const taskCustomFieldDefinitionsRelations = relations(taskCustomFieldDefinitions, ({ one, many }) => ({
  organization: one(organizations, {
...
export const boardTaskCustomFieldsRelations = relations(boardTaskCustomFields, ({ one }) => ({
  board: one(boards, {
...
export const gatewayInstalledSkillsRelations = relations(gatewayInstalledSkills, ({ one }) => ({
  gateway: one(gateways, {
