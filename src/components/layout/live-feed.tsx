...
export function LiveFeed() {
  const { logs, sessions, activities, toggleLiveFeed, liveFeedOpen } =
    useMissionControl();
  const [expanded, setExpanded] = useState(true);

  if (!liveFeedOpen) return null;
  ...
  if (!expanded) {
    return (
      <div className="w-10 bg-card border-l border-border flex flex-col items-center py-3 shrink-0 hidden md:flex">
        <button
          onClick={() => setExpanded(true)}
          className="w-8 h-8 rounded-md text-muted-foreground hover:text-foreground hover:bg-secondary transition-smooth flex items-center justify-center p-0"
  ...
  return (
    <div className="w-72 h-full bg-card border-l border-border flex-col shrink-0 slide-in-right hidden xl:flex">
      {/* Header */}
          ...
          {sessions
            ...
            .map((session: any) => (
              ...
              >
                ...
                <span className="text-foreground truncate flex-1 font-mono-tight">
                  {session.key || session.id}
                </span>
                <span className="text-muted-foreground">
                  {session.model?.split("/").pop()?.slice(0, 8)}
                </span>
              </div>
            ))}
          {sessions.filter((s: any) => s.active).length === 0 && (
  ...
  return (
    <div className="px-3 py-2 hover:bg-secondary/50 transition-smooth group">
      <div className="flex items-start gap-2">
        <div
          className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${levelIndicator}`}
        />
